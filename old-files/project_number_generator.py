...
class MonitorNetwork:
    ...
    def get_packets_receive(self, average_of_tries: int) -> int:
        ...
        try:
            ...
            assert (
                not len(self.average_packets_receive) == 0
            ), "<< WARNING >> Not enough packets to continue."
            packet = sum(self.average_packets_receive) / len(
                self.average_packets_receive
            )
...
class InteractUser:
    def __init__(self):
        self.welcome_phrase: str = (
            "<| Generate 16-bit integer with the network statistics |>"
        )
        self.network_packets: str = (
            "Insert the number of packets to acquire random number from:"
        )
    ...
    def interact(self) -> int:
        ...
        while True:
            try:
                ...
                assert (
                    not packet_number < 0
                ), "<< WARNING >> Please insert number greater than zero ( 0 )."
